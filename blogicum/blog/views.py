...
def get_posts(posts=Post.objects, filters=True, annotations=True):
    ...
    """Получить посты. filters и annotations - для условий."""
...
def profile(request, user_name):
    ...
    """Профиль пользователя. Посты пагинируются."""

    user = get_object_or_404(User, username=user_name)
    posts = get_posts(filters=False).filter(author=user)
    paginator = Paginator(posts, NUMBER_POSTS) # Пагинация
